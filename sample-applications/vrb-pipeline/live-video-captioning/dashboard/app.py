...
class StartRunRequest:
    ...
    Attributes:
        ...
        detectionModelName: The name of the detection model to use.
    ...
    detectionModelName: str = "yolov8s"
...
def _discover_detection_models(root: Path) -> list[str]:
    if not root.exists():
        return []
    models: list[str] = []
...
async def list_detection_models(request: web.Request) -> web.Response:
    """
    ---
    summary: List available object detection models
    description: Returns a list of available object detection models from the models directory.
    tags:
      - Models
    responses:
      "200":
        description: List of available object detection models
        content:
          application/json:
            schema:
              type: object
              properties:
                models:
                  ...
                  items:
                    type: string
                  description: List of object detection model names
    """
    models = _discover_detection_models(DETECTION_MODELS_DIR)
    return web.json_response({"models": models})
...
No rate limiting is applied.
    ...
    swagger.add_routes([
        ...
        web.get("/api/detection-models", list_detection_models, allow_head=False),
